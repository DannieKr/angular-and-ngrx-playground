...
const editArticleFeature = createFeature({
    ...
    reducer: createReducer(
        ...
        on(editArticleActions.getArticleFailure, (state, action) => ({ ...state, isLoading: false})),

        on(editArticleActions.updateArticle, (state) => ({ ...state, isSubmitting: true })),
        on(editArticleActions.updateArticleSuccess, (state, action) => ({ ...state, isSubmitting: false })),
